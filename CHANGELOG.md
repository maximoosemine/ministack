...
- **DynamoDB Streams — backlogs and ESM read positions leaked, degrading a long-lived container** — stream records were never trimmed (AWS expires them after 24 hours), `DeleteTable` did not drop a table's backlog, and `DeleteEventSourceMapping` left the ESM's stream position, Kinesis positions, and backoff entry behind, so a container cycling stacks left the poller walking a growing set on every tick until stream→Lambda delivery slowed. Records now expire 24 hours after they are written, as on AWS, and both deletes release their state — from the CloudFormation paths as well as the API ones. Every read position — Lambda ESM pollers, EventBridge Pipes, and the shard iterators handed to `dynamodbstreams` clients — became an absolute stream position tracked against a trim horizon, so expiring records never shift a consumer's place in the stream, and a consumer behind the horizon resumes there rather than replaying a stale offset. Retention is wall-clock, so an idle stream ages out on read as well as on write.
